...
export default async function TownDetailPage({
  ...
  return (
    <main className="pixel-town-page">
      <LiveTownWorld
        townId={town.id}
      ...
      <details className="pixel-ledger">
        <summary className="pixel-ledger-sum">
          ▾ TOWN LEDGER · census, fingerprint, minds, history
        </summary>
        <div className="pixel-ledger-body">
          <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
            ...
            <div className="flex flex-wrap gap-2">
              <Link href={`/towns/${town.id}/rehearse`} className="pixel-btn pixel-btn-ghost">
                CUSTOM QUEST
              </Link>
              {canResync && <ResyncButton townId={town.id} />}
              {repoUrl && (
                <a href={repoUrl} target="_blank" rel="noreferrer" className="pixel-btn pixel-btn-ghost">
                  GITHUB
                </a>
              )}
            </div>
